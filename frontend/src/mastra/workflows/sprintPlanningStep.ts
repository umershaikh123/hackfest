...
export const sprintPlanningStep = createStep({
  ...
  execute: async ({ inputData, runtimeContext }) => {
    try {
      console.log(`🏃‍♂️ Starting sprint planning for ${inputData.productTitle}...`)

      // Validate environment for Linear integration
      if (inputData.createLinearProject) {
        if (!process.env.LINEAR_API_KEY) {
          console.warn("⚠️ Linear integration requested but LINEAR_API_KEY not found")
        }
        if (!inputData.linearTeamId) {
          console.warn("⚠️ Linear integration requested but linearTeamId not provided")
        }
      }

      // Prepare sprint planning message for the agent
      const sprintPlanningMessage = `
        Please create a comprehensive sprint plan for "${inputData.productTitle}".
        ...
        ${inputData.linearTeamId ? `- Linear Team ID: ${inputData.linearTeamId}` : ''}
        ...
        ${inputData.createLinearProject ? '5. Set up Linear cycles and issues for project management' : ''}

        Focus on creating a sustainable development velocity that the team can maintain.
      `

      // Execute sprint planning with the agent
      const agentResponse = await sprintPlannerAgent.generate(sprintPlanningMessage, {
        toolData: {
          sprintPlannerTool: {
      ...
      if (agentResponse.toolCalls && agentResponse.toolCalls.length > 0) {
        const sprintToolCall = agentResponse.toolCalls.find(call => call.toolName === 'sprintPlannerTool')
        if (sprintToolCall) {
      ...
      if (!sprintPlanResult) {
        console.warn("⚠️ No sprint planner tool result found, creating fallback response")
        sprintPlanResult = {
          sprints: [],
          summary: {
            totalStoryPoints: inputData.userStories.reduce((sum, story) => sum + story.storyPoints, 0),
      ...
      console.log(`📊 Generated ${sprintPlanResult.sprints.length} sprints with ${sprintPlanResult.summary.sprintVelocity} velocity`)
      
      if (sprintPlanResult.linearIntegration.enabled) {
        console.log(`🔗 Linear integration: ${sprintPlanResult.linearIntegration.cyclesCreated?.length || 0} cycles created`)
      }
...
export async function testSprintPlanningStep() {
    ...
    features: [
      {
        ...
        acceptanceCriteria: ["Activity input form", "Emission calculations", "Daily/weekly/monthly views"],
        priority: "high" as const,
      },
      {
        name: "Green Challenges",
        description: "Community challenges to reduce environmental impact",
        acceptanceCriteria: ["Challenge creation", "Progress tracking", "Leaderboards"],
    ...
    userStories: [
      {
        ...
        acceptanceCriteria: ["Transportation type selection", "Distance input", "Emission calculation"],
        priority: "high" as const,
        storyPoints: 8,
      },
      {
        ...
        acceptanceCriteria: ["Charts and graphs", "Trend analysis", "Goal progress"],
        priority: "medium" as const,
        storyPoints: 5,
      },
    ],
  ...
  try {
    ...
    console.log(`🔗 Linear integration: ${result.linearIntegration.enabled ? 'Enabled' : 'Disabled'}`)
