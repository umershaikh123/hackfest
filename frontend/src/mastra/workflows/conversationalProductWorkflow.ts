...
const ConversationalWorkflowInput = z.object({
  ...
  notionDatabaseId: z
    .string()
    .optional()
    .describe("Notion database ID for PRD publishing"),
  linearTeamId: z
    .string()
    .optional()
    .describe("Linear team ID for project creation"),

  // Workflow configuration
  enableSprintPlanning: z
    .boolean()
    .default(true)
    .describe("Whether to generate sprint plans"),
  enableVisualDesign: z
    .boolean()
    .default(true)
    .describe("Whether to create visual workflows"),
  createLinearProject: z
    .boolean()
    .default(false)
    .describe("Whether to create Linear cycles"),

  // Team settings
  teamSize: z
    .number()
    .default(4)
    .describe("Development team size for sprint planning"),
  sprintLength: z
  ...
  userFeedback: z
    ...
    .optional()
    .describe("User feedback for iterative refinement"),
  iterationMode: z
    .boolean()
    .default(false)
    .describe("Whether this is an iteration cycle"),
  ...
  sessionId: z
    .string()
    .optional()
    .describe("Existing session ID for continuation"),
  previousResults: z
    .record(z.any())
    .optional()
    .describe("Previous workflow results for context"),
})
...
const initializeSessionStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    let workflowMode: "initial_run" | "feedback_iteration" | "step_refinement" =
      "initial_run"
    ...
    if (inputData.userFeedback && inputData.userFeedback.length > 0) {
      workflowMode = inputData.iterationMode
        ? "feedback_iteration"
        : "step_refinement"
      ...
      for (const feedback of inputData.userFeedback) {
        try {
          ...
          const routingDecision: RoutingDecision = {
            targetAgent:
              feedback.type === "idea_refinement"
                ? "idea_generation"
                : feedback.type === "user_story_modification"
                ? "user_story_generator"
                : feedback.type === "prd_revision"
                ? "prd_agent"
                : feedback.type === "sprint_adjustment"
                ? "sprint_planner"
                : feedback.type === "visual_design_change"
                ? "visual_design"
          ...
          if (
            routingDecision.targetAgent === "idea_generation" &&
            !targetSteps.includes("ideaGeneration")
          ) {
            targetSteps.unshift("ideaGeneration")
          }
          if (
            routingDecision.targetAgent === "user_story_generator" &&
            !targetSteps.includes("userStoryGeneration")
          ) {
    ...
    console.log(
      `🚀 Initializing ${workflowMode} workflow for session ${sessionId}`
    )
    ...
    return {
      ...
      workflowMode,
      targetSteps,
      feedbackRoutingResults:
        feedbackRoutingResults.length > 0 ? feedbackRoutingResults : undefined,
    }
  },
})
...
export const conversationalProductWorkflow = createWorkflow({
  ...
  description:
    "Complete conversational product development workflow with feedback routing and iterative refinement",
  inputSchema: ConversationalWorkflowInput,
  ...
  .map(async ({ inputData, getStepResult }) => {
    ...
    const sprintResult = inputData.enableSprintPlanning
      ? getStepResult(sprintPlanningStep)
      : null
    const visualResult = inputData.enableVisualDesign
      ? getStepResult(visualDesignStep)
      : null
    ...
    if (sessionInit.workflowMode === "feedback_iteration") {
      ...
      status = visualResult.success
        ? "visual_design_completed"
        : "visual_design_failed"
      currentStep = visualResult.success
        ? "visual-design-completed"
        : "visual-design-failed"
    } else if (sprintResult && inputData.enableSprintPlanning) {
      status =
        sprintResult.sprints.length > 0
          ? "sprint_planning_completed"
          : "sprint_planning_failed"
      currentStep =
        sprintResult.sprints.length > 0
    ...
    const canIterateOn = [
      ...
      ...(inputData.enableVisualDesign
        ? ["Visual Workflows & User Journeys"]
        : []),
    ]
    ...
    const maxPossibleArtifacts =
      2 +
      (inputData.enableSprintPlanning ? 1 : 0) +
      (inputData.enableVisualDesign ? 1 : 0) +
      (prdResult ? 1 : 0)
    const completionPercentage = Math.round(
      (artifactsGenerated / maxPossibleArtifacts) * 100
    )
    ...
    return {
      ...
      sprintPlanningAnalysis:
        sprintResult && inputData.enableSprintPlanning
          ? {
              ...
              linearIntegration: {
                ...
                cyclesCreated:
                  sprintResult.linearIntegration.cyclesCreated?.length,
                issuesCreated:
                  sprintResult.linearIntegration.issuesCreated?.length,
              },
      ...
      visualDesignAnalysis:
        visualResult && inputData.enableVisualDesign
          ? {
...
export async function testConversationalWorkflow() {
  console.log(
    "🚀 Testing Conversational Product Workflow with All Integrations"
  )
  ...
  const result = await run.start({
    inputData: {
      rawIdea:
        "A mindfulness app that combines meditation with habit tracking and social accountability features",
      additionalContext:
        "Target busy professionals who want to build better mental health habits but struggle with consistency. Should integrate with calendar apps.",
  ...
  console.log(
    "📈 Completion:",
    `${result.qualityMetrics.completionPercentage}%`
  )
  console.log(
    "🔗 Integrations Used:",
    result.qualityMetrics.integrationsUsed.join(", ")
  )
  console.log(
    "📋 Artifacts Generated:",
    result.qualityMetrics.artifactsGenerated
  )
  ...
  if (result.sprintPlanningAnalysis) {
    ...
    console.log(
      "  📊 Sprints Generated:",
      result.sprintPlanningAnalysis.sprintsGenerated
    )
    console.log(
      "  🔗 Linear Integration:",
      result.sprintPlanningAnalysis.linearIntegration.enabled
    )
  }
  ...
  if (result.visualDesignAnalysis) {
    ...
    console.log(
      "  📊 Visualizations:",
      result.visualDesignAnalysis.visualizations.length
    )
    console.log(
      "  🔗 Miro Boards:",
      result.visualDesignAnalysis.visualizations
        .map(v => v.miroBoard.viewLink)
        .join(", ")
    )
  }
  ...
  console.log(
    "\n💡 Next Steps:",
    result.conversationalContext.nextStepSuggestions.slice(0, 3).join(", ")
  )
  console.log(
    "🔄 Can Iterate On:",
    result.conversationalContext.canIterateOn.join(", ")
  )
...
export async function testFeedbackIteration() {
  ...
  const feedbackExample: Feedback = {
    ...
    content:
      "I want to add a feature for team challenges where colleagues can create mindfulness challenges together",
    priority: "high",
  ...
  console.log(
    "  📝 Feedback Processed:",
    result.feedbackProcessing?.feedbackReceived.length || 0
  )
  console.log(
    "  🎯 Routing Decisions:",
    result.feedbackProcessing?.routingDecisions.length || 0
  )
