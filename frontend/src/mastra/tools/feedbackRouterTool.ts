...
export const feedbackRouterTool = createTool({
  ...
  execute: async ({ context, runtimeContext }) => {
    ...
    switch (feedbackType) {
      case "idea_refinement":
        routingDecision = {
          ...
          reasoning:
            "User wants to refine the core product idea or add/modify features",
          actionRequired:
            "Re-analyze and enhance the product idea based on user feedback",
          contextToPass: {
        ...
        if (
          feedbackContent.includes("completely different") ||
          feedbackContent.includes("start over")
        ) {
          routingDecision.shouldSuspendWorkflow = true
          requiresUserConfirmation = true
          additionalRecommendations.push(
            "Consider if this requires a complete restart of the workflow"
          )
        }
        break
      ...
      case "user_story_modification":
        routingDecision = {
          ...
          actionRequired:
            "Update user stories based on feedback while maintaining consistency",
          contextToPass: {
        ...
        if (
          feedbackContent.includes("priority") ||
          feedbackContent.includes("points")
        ) {
          additionalRecommendations.push(
            "Update sprint planning to reflect story changes"
          )
        }
        break
      ...
      case "prd_revision":
        routingDecision = {
          ...
          actionRequired:
            "Revise PRD sections based on user feedback and republish if necessary",
          contextToPass: {
        ...
        if (currentWorkflowState.prdStatus === "published") {
          additionalRecommendations.push(
            "Will create an updated version of the published PRD"
          )
      ...
      case "sprint_adjustment":
        routingDecision = {
          ...
          reasoning:
            "User wants to adjust sprint planning, timeline, or resource allocation",
          actionRequired:
            "Update sprint plans based on feedback and sync with Linear if configured",
          contextToPass: {
        ...
        if (
          feedbackContent.includes("timeline") ||
          feedbackContent.includes("deadline")
        ) {
          additionalRecommendations.push(
            "Review all sprint timelines for consistency"
          )
      ...
      case "visual_design_change":
        routingDecision = {
          ...
          reasoning:
            "User wants changes to visual designs, wireframes, or user journey maps",
          actionRequired:
            "Update visual artifacts and Miro boards based on feedback",
          contextToPass: {
        ...
        if (
          feedbackContent.includes("user flow") ||
          feedbackContent.includes("journey")
        ) {
          additionalRecommendations.push(
            "Consider updating user stories to align with new user flows"
          )
        }
        break
      ...
      case "general_clarification":
        ...
        if (
          feedbackContent.includes("feature") ||
          feedbackContent.includes("idea")
        ) {
          routingDecision = {
            targetAgent: "idea_generation",
            reasoning:
              "Clarification request related to product features or core idea",
            actionRequired:
              "Provide detailed clarification and enhance documentation",
            shouldSuspendWorkflow: true,
          }
        } else if (
          feedbackContent.includes("story") ||
          feedbackContent.includes("user")
        ) {
          routingDecision = {
            targetAgent: "user_story_generator",
            reasoning:
              "Clarification request related to user stories or personas",
            actionRequired:
              "Clarify user story details and provide additional context",
            shouldSuspendWorkflow: true,
          }
        } else {
          routingDecision = {
            ...
            reasoning:
              "General clarification that requires workflow-level understanding",
            actionRequired:
              "Provide comprehensive clarification and next steps",
            shouldSuspendWorkflow: true,
          }
        }
        requiresUserConfirmation = true
      ...
      case "workflow_direction":
        routingDecision = {
          targetAgent: "workflow_orchestrator",
          reasoning:
            "User wants to change workflow direction or skip/repeat steps",
        ...
        if (feedbackContent.includes("skip")) {
          additionalRecommendations.push(
            "Ensure skipped steps don't break downstream dependencies"
          )
        }
        break
      ...
      default:
        ...
        routingDecision = {
          targetAgent: "workflow_orchestrator",
          reasoning:
            "Feedback type unclear, routing to orchestrator for analysis",
          actionRequired:
            "Analyze feedback and determine appropriate next steps",
          shouldSuspendWorkflow: true,
        }
        requiresUserConfirmation = true
    }
    ...
    if (
      feedbackContent.includes("integration") ||
      feedbackContent.includes("api")
    ) {
      additionalRecommendations.push(
        "Consider technical implications for integration planning"
      )
    }

    if (
      feedbackContent.includes("user experience") ||
      feedbackContent.includes("ux")
    ) {
      additionalRecommendations.push(
        "Review visual design and user story consistency"
      )
    }
